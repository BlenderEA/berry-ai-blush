...
const PremiumTokenHero = () => {
    ...
    const fetchPriceData = async () => {
      try {
        ...
        if (data && data.pairs && data.pairs.length > 0) {
          ...
          setRealStats({
            holders: Math.floor(Math.random() * 100) + 950,
            // DexScreener doesn't provide holder count, so we'll estimate
  ...
  return <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-dark via-dark-lighter to-dark border border-berry/30 p-8 mb-12">
      ...
      <div className="absolute inset-0 opacity-20">
        ...
        <div className="absolute bottom-1/4 left-0 w-80 h-80 rounded-full bg-berry-purple/30 blur-[100px] animate-float" style={{
        animationDelay: '2s'
      }}></div>
      </div>
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-8">
          <div>
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h1 className="text-4xl md:text-6xl font-bold gradient-text animate-shimmer text-zinc-50">
                  $BUSTYBERRY
                </h1>
          ...
          {priceData && <Card className="glass-card border-berry/30 min-w-[300px]">
              <CardContent className="p-6">
                <div className="text-center">
                  ...
                  <div className={`flex items-center justify-center gap-2 ${priceData.priceChange?.h24 >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                    {priceData.priceChange?.h24 >= 0 ? <TrendingUp className="h-5 w-5" /> : <TrendingDown className="h-5 w-5" />}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3 mb-6">
              {quickBuyAmounts.map(amount => <a key={amount} href={`https://jup.ag/swap/SOL-${contractAddress}`} target="_blank" rel="noopener noreferrer">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <a href={`https://jup.ag/swap/SOL-${contractAddress}`} target="_blank" rel="noopener noreferrer" className="flex-1">
                <Button className="w-full berry-button">
                  Buy on Jupiter
                  <ExternalLink className="ml-2 h-4 w-4" />
                </Button>
              </a>
              <a href={`https://raydium.io/swap/?inputCurrency=sol&outputCurrency=${contractAddress}`} target="_blank" rel="noopener noreferrer" className="flex-1">
          ...
          <div>
            ...
            <div className="p-4 rounded-lg bg-dark-lighter/50 border border-dark-border hover:border-berry/30 transition-colors">
              <div className="flex justify-between items-center mb-2">
                ...
                <Button variant="outline" size="sm" onClick={copyToClipboard} className="bg-berry/20 hover:bg-berry/30 border-berry/30">
